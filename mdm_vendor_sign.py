...
def checkResult(ok_str, b):
    ouputStr = b.decode()
    if ouputStr.rstrip().split('\n')[0] == ok_str:
        p('OK\n')
        return True
    else:
        p('FAILED\n')
...
def mdm_vendor_sign():
    ...
    command = subprocess.Popen(args, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
    output, error = command.communicate(input=csr_file.encode('utf-8'))
    if not checkResult('verify OK', output):
        return
    ...
    command = subprocess.Popen(args, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.STDOUT)
    output, error = command.communicate(input=key_file.encode('utf-8'))
    if not checkResult('RSA key ok', output):
    ...
    process = subprocess.Popen(['curl', 'https://developer.apple.com/certificationauthority/AppleWWDRCA.cer'],
